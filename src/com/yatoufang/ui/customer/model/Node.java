package com.yatoufang.ui.customer.model;
...
public class Node implements Paintable {
    ...
    public boolean pointInNode(int x0, int y0) {
        return x0 > x - width / 2 && x0 < x + width / 2 && y0 > y - height / 2 && y0 < y + height / 2;
    }
    ...
    public void animateToStatic() {
        ...
        taskTimer = new Timer();
        taskTimer.schedule(task, options.getTimeout() * 1000L);
        if (this.moving) {
            return;
        }
        this.moving = true;
    ...
    public void removeNode() {
        ArrayList<Node> oldNodes = new ArrayList<>(this.mindmap.getNodes());
        ArrayList<Line> oldLines = new ArrayList<>(this.mindmap.getLines());
